...
import type { Location, AmenityType, LocationLabelType } from '@/services/types/locations.dto';
import type { Club } from '@/services/types/clubs.dto';
import SafeDate from '@/app/components/ui/SafeDate/SafeDate';
import SportsBadges from '@/app/components/ui/SportsBadges/SportsBadges';
...
export default function LocationDetails({ location, club }: Props) {
  const t = useTranslations('locationDetails');
  const tStatus = useTranslations('locationDetails.status');
  const tLabels = useTranslations('locationLabels');
  const tAmenities = useTranslations('locationAmenities');
  ...
  const amenitiesText = useMemo(() => {
    ...
    return (amenities as AmenityType[])
      .map((a) => {
        try {
          const tr = tAmenities(a);
          if (!tr || tr === a || tr.includes('.')) return a;
          return tr;
        } catch {
          return a;
        }
  ...
  const renderLabel = (raw: unknown) => {
    const v = String(raw ?? '').trim() as LocationLabelType | '';
    if (!v || v === 'None') return '—';
    try {
      const tr = tLabels(v);
      if (!tr || tr === v || tr.includes('.')) return v;
      return tr;
    } catch {
      return v;
    }
  ...
  return (
    <section className={styles.wrapper}>
      ...
      <div className={styles.card}>
        ...
        <div className={styles.row}>
          <div className={styles.label}>{t('labels.club')}</div>
          <div className={styles.value}>
        ...
        <div className={styles.row}>
          <div className={styles.label}>{t('labels.label')}</div>
          <div className={styles.value}>{renderLabel(label)}</div>
        </div>

        <div className={styles.row}>
          <div className={styles.label}>{t('labels.order')}</div>
          <div className={styles.value}>{typeof order === 'number' ? order : '—'}</div>
        </div>

        <div className={styles.row}>
          <div className={styles.label}>{t('labels.price')}</div>
          <div className={styles.value}>{priceText?.trim() ? priceText : '—'}</div>
        </div>

        <div className={styles.row}>
          <div className={styles.label}>{t('labels.sports')}</div>
          <div className={styles.value}>
            <SportsBadges sports={location.sports} sportTypes={location.sportTypes} />
          </div>
        </div>
        ...
        <div className={styles.row}>
          <div className={styles.label}>{t('labels.amenities')}</div>
          <div className={styles.value}>{amenitiesText}</div>
        </div>
        ...
        <div className={`${styles.row} ${styles.rowDescription}`}>
          <div className={styles.label}>{t('labels.workingHours')}</div>
          ...
          <div className={styles.value}>
            <button
              ...
              onClick={() => setShowSchedule((v) => !v)}
            >
              {showSchedule ? t('workingHoursToggle.hide') : t('workingHoursToggle.show')}
            </button>
        ...
        <div className={styles.row}>
          <div className={styles.label}>{t('labels.created')}</div>
          <div className={styles.value}>
            <SafeDate value={location.createdAt} />
          </div>
        </div>

        <div className={styles.row}>
          <div className={styles.label}>{t('labels.updated')}</div>
          <div className={styles.value}>
            <SafeDate value={location.updatedAt} />
          </div>
        </div>
      </div>
    </section>
