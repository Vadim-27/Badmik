...
export default function SalaryField<TFieldValues extends FieldValues>({
  ...
  const t = useTranslations('SalaryField');
  ...
  const errNonNegative = t('errors.nonNegative');
  ...
  const RateSuffix = (
    <div className={styles.suffix}>
      <select
        className={styles.currency}
        value={(currencyField.value as Currency) ?? 'UAH'}
        onChange={(e) => currencyField.onChange(e.target.value as FieldPathValue<TFieldValues, Path<TFieldValues>>)}
        aria-label="currency"
      >
  ...
  const placeholderRate = t('placeholders.rate');
  ...
  const renderRateInput = () => {
    ...
    const label =
      type === 'Hourly'
        ...
        : type === 'Salary'
          ? t('rateLabels.Salary')
          : t('rateLabels.PerTraining');

    const ctrl = type === 'Hourly' ? hourly : type === 'Salary' ? monthly : perTraining;
      ...
      <>
        ...
        <div className={styles.inputWithSuffix}>
          <input
            ...
            className={`${styles.input} ${styles.inputPadRight} ${ctrl.fieldState.error ? styles.errorInput : ''}`}
            value={(ctrl.field.value as number | string | null | undefined) ?? ''}
            onChange={(e) => handleRateChange(e, ctrl.field.onChange)}
          />
          {RateSuffix}
        </div>
        {ctrl.fieldState.error && <p className={styles.errorText}>{String(ctrl.fieldState.error.message)}</p>}
      </>
    );
  };

  const typeOptions: { value: SalaryType; label: string }[] = [
    { value: 'Hourly', label: t('types.Hourly') },
    { value: 'Salary', label: t('types.Salary') },
    { value: 'PerTraining', label: t('types.PerTraining') },
  ];

  return (
    <div className={styles.salaryBlock}>
      <div>
        <label className={styles.label}>{t('payTypeLabel')}</label>

        <div className={styles.chipsRow} role="radiogroup" aria-label={t('payTypeAria')}>
          {typeOptions.map((opt) => {
            const active = (salaryTypeField.value as SalaryType) === opt.value;
            return (
              <button
                ...
                aria-checked={active}
                className={`${styles.chip} ${active ? styles.chipActive : ''}`}
                onClick={() => salaryTypeField.onChange(opt.value as FieldPathValue<TFieldValues, Path<TFieldValues>>)}
              >
                {opt.label}
              </button>
            );
