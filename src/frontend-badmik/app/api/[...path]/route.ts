...
export async function forward(req: NextRequest) {
  ...
  try {
    ...
    // ✅ форвардимо заголовки майже як є
    const headers = new Headers(req.headers);
    headers.delete('host');
    headers.delete('connection');
    headers.delete('content-length');    // хай fetch сам
    headers.delete('accept-encoding');   // щоб не було gzip-стріма проблем

    headers.set('x-request-id', reqId);
    if (token) headers.set('authorization', `Bearer ${token}`);

    // ✅ важливо: body стрімом
    ...
    if (hasBody) {
      // NextRequest.body може бути null — ок
      init.body = req.body as any;

      // ✅ Node fetch вимагає duplex для stream body
      // TS може лаятись — тому any
